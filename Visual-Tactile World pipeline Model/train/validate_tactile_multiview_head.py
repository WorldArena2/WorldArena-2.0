...
def main():
    ...
    # Infer action modalities dims from a sample if action expert is enabled
    action_modalities_dims = None
    if args.enable_action_expert:
        sample_item = dataset[0]
        action_modalities_dims = {
        ...
        print(f"[ActionExpert] Inferred modalities: {action_modalities_dims}")
    ...
    model = WanTactile(
        ...
        action_modalities_dims=action_modalities_dims,
    )
    ...
    for idx in range(args.num_samples):
        ...
        if pred_action is not None:
            print(f"  Saved pred: action with shape {pred_action.shape}", end="")
            np.save(os.path.join(args.output_dir, f"{prefix}_pred_action.npy"), pred_action.float().cpu().numpy())
            print(" + action", end="")
        print()
