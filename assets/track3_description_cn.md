...
我们鼓励参赛的世界模型（WAM）同时尝试 AgileX 上的**纯视觉任务**与**视觉-触觉任务**，以及 Franka 上的纯视觉任务，以展示在视觉与接触反馈下的真实世界泛化操作能力。Track 3 最终设有**视觉榜、触觉榜与总榜**三个榜单；仅视触同时参加的团队进入总榜，总榜按视觉 : 触觉 = **7 : 3** 加权（详见下文「评价指标与榜单」）。
...
AgileX 上每个任务约包含 **100 条**演示数据。Franka 则为上述三个纯视觉任务提供演示数据。
...
每个任务通过其**真机执行成功率**进行评价。如果机器人在允许的步数内完成整个任务且未触发安全干预，则该次尝试视为成功。各方向内的得分可按任务难度进行归一化后聚合。

Track 3 最终设置 **三个榜单**：

| 榜单 | 覆盖任务 | 入榜条件 |
|---|---|---|
| **视觉榜** | 纯视觉任务（AgileX 纯视觉 + Franka 纯视觉） | 参加视觉方向评测的团队 |
| **触觉榜** | 视觉-触觉任务（AgileX 视觉-触觉） | 参加触觉方向评测的团队 |
| **总榜** | 视觉 + 触觉 | **仅视触同时参加**的团队 |

总榜分数按视觉与触觉得分 **7 : 3** 加权：

```text
总榜分数 = 0.7 × 视觉得分 + 0.3 × 触觉得分
```

仅参加单一方向的团队会出现在对应分榜上，但**不会**记入总榜。
...
- 参赛队伍可提交任意任务 / 平台子集；可只参加视觉或触觉分榜。
- 若希望进入**总榜**，须**同时参加视觉与触觉**两个方向；总榜按视觉 : 触觉 = **7 : 3** 加权。
