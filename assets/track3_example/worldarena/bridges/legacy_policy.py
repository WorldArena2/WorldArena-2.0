...
from worldarena.geometry import (
    cont6d_to_matrix,
...
def _populate_legacy_manifold_arm_fields(
    new_obs: Dict[str, Any],
    left: Any,
    right: Any,
) -> None:
    """Per-arm keys for ViTAL ``end_pose_eef6d`` (``extract_single_arm_eef6d_from_obs``)."""
    if left is not None:
...
def observation_packet_to_new_obs(
    ...
    *,
    ...
    _populate_legacy_manifold_arm_fields(new_obs, left, right)
...
def action_packet_looks_like_misparsed_joint(packet: ActionPacket) -> bool:
    ...
    from worldarena.protocol import ACTION_MODE_JOINT_ABSOLUTE
...
def parse_policy_infer_full(
    ...
    *,
    ...
    from worldarena.action_debug import PolicyInferResult, as_actions_array
