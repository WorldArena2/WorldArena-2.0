...
from worldarena.protocol import CAMERA_ROLE_GLOBAL
from worldarena.schema import CameraObservation, ObservationPacket
...
def stack_camera_frames(cam: CameraObservation):
    ...
    for blob in cam.frame_history_bytes or []:
        ...
        if h <= 0 or w <= 0:
            continue
        frames.append(np.frombuffer(blob, dtype=np.uint8).reshape(h, w, 3))
